...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            AnsiConsole.WriteLine("3. Eliminar un elemento del diccionario.");
    ...
    static void DeleteEmployees(Dictionary<int, Employee> employees)
    {
        if (employees.Count == 0)
        {
            AnsiConsole.WriteLine("No hay empleados para eliminar.");
            return;
        }

        var deleteId = AnsiConsole.Ask<int>("Ingrese el ID del empleado que desea eliminar: ");

        if (employees.ContainsKey(deleteId))
        {
            employees.Remove(deleteId);
            AnsiConsole.WriteLine("Empleado eliminado con éxito.");
        }
        else
        {
            AnsiConsole.WriteLine("No se encontró ningún empleado con el ID proporcionado.");
        }
    }
}